var springChartApp = (function() {
	...
	var App = {
		...
		addResultChart: function(initValue) {
			this.resultChart = UpDownChart.init('right_chart_div', [{
				'base': 'One',
				'value': initValue,
				baseValue: initValue,
				color: COLOR.UP,
	...
	var UpDownChart = {
		...
		init: function(containerId, initDataData, min, max) {
	...
	var MainChart = {
		...
		init: function(containerId, initValue) {
	...
	var TrendLine = {
		...
		init: function(parentChart, max, min, basePoint) {
	...
	return {
		start: function() {
			...
			function dataReceiveHandler(priceData) {
				if (App.hasNotMainChart()) {
					App.addMainChart(MainChart.init('main_chart_div', priceData.avgPrice));
					App.addBaseChart(UpDownChart.init('base_chart_div', [{
						'base': 'One',
